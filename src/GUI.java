...
public class GUI extends JFrame {
	 ...
	 * exiting
	 */
	private JMenu option, settings, changeColor;
	...
	private int southTricksTaken, eastTricksTaken, northTricksTaken, westTricksTaken, southTricksBid, eastTricksBid,
			westTricksBid, northTricksBid, ptsScoredN, ptsScoredS, ptsScoredE, ptsScoredW, scoreN, scoreS, scoreE,
			scoreW, turnNumber, nextPlayer, imageNum, numHumans, screenResNum;
	 ...
	 */
	public GUI(int screenRes, int scoreN, int scoreS, int scoreE, int scoreW) {
		// Set the layout of the JFrame to Border Layout
		setLayout(new BorderLayout());
	...
	private void initializeVariables(int screenRes) {
		...
		horizontalHandIcon = new ImageIcon("src/back/13h.jpg");
		horizontalHandImage = horizontalHandIcon.getImage();
		verticalHandIcon = new ImageIcon("src/back/13v.jpg");
	...
	private void updateCardImages() {
		if (screenResNum == 1) {
			horizontalHandIcon = new ImageIcon("src/back/" + --imageNum + "h.jpg");
			horizontalHandImage = horizontalHandIcon.getImage();
			sizedHorizontalHandImage = horizontalHandImage.getScaledInstance((50 + imageNum * 25), 100,
					Image.SCALE_FAST);

			verticalHandIcon = new ImageIcon("src/back/" + imageNum + "v.jpg");
			verticalHandImage = verticalHandIcon.getImage();
	...
	private class NextTurn implements ActionListener {
		...
		private void showWin(String msg){
			endOfGameWinner = new JLabel(msg);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (driver.players[0].getHand().size() == 0) {
				...
				model.setValueAt(((Integer) northTricksBid).toString(), 0, 1);
				model.setValueAt(((Integer) southTricksBid).toString(), 0, 2);
				model.setValueAt(((Integer) eastTricksBid).toString(), 0, 3);
				model.setValueAt(((Integer) westTricksBid).toString(), 0, 4);
				model.setValueAt(((Integer) northTricksTaken).toString(), 1, 1);
				model.setValueAt(((Integer) southTricksTaken).toString(), 1, 2);
				model.setValueAt(((Integer) eastTricksTaken).toString(), 1, 3);
				model.setValueAt(((Integer) westTricksTaken).toString(), 1, 4);

				ptsScoredN = pointsScored(northTricksBid, northTricksTaken);
				ptsScoredS = pointsScored(southTricksBid, southTricksTaken);
				ptsScoredE = pointsScored(eastTricksBid, eastTricksTaken);
				ptsScoredW = pointsScored(westTricksBid, westTricksTaken);
				model.setValueAt(((Integer) ptsScoredN).toString(), 2, 1);
				model.setValueAt(((Integer) ptsScoredS).toString(), 2, 2);
				model.setValueAt(((Integer) ptsScoredE).toString(), 2, 3);
				model.setValueAt(((Integer) ptsScoredW).toString(), 2, 4);
				scoreN += ptsScoredN;
				scoreS += ptsScoredS;
				scoreE += ptsScoredE;
				scoreW += ptsScoredW;
				model.setValueAt(((Integer) scoreN).toString(), 3, 1);
				model.setValueAt(((Integer) scoreS).toString(), 3, 2);
				model.setValueAt(((Integer) scoreE).toString(), 3, 3);
				model.setValueAt(((Integer) scoreW).toString(), 3, 4);
				...
				if (scoreN >= 200) {
					showWin("Congratuations N! You win!");
				} else if (scoreS >= 200) {
					showWin("Congratuations S! You win!");
				} else if (scoreE >= 200) {
					showWin("Congratuations E! You win!");
				} else if (scoreW >= 200) {
					showWin("Congratuations W! You win!");
				} else {
	...
	private class NextHand implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			dispose();
			GUI gui = new GUI(screenResNum, scoreN, scoreS, scoreE, scoreW);
			gui.display();
		}
	}
	...
	private class MyModel extends AbstractTableModel {
		...
		String[][] data = { { "Tricks Bid", "", "", "", "" }, { "Tricks Taken", "", "", "", "" },
